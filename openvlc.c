...
static int ACK_TIMEOUT = 10; // Ack length is 10 bytes: times*bytes*bits_per_byte*manchester
    ...
    _Bool *buffer_after_coding, int len_before_coding)
{
    ...
    for (byte_index=PREAMBLE_LEN; byte_index<len_before_coding-1; byte_index++) {
        mask = 0x80;
        curr_byte = buffer_before_coding[byte_index] & 0xff;
        while (mask) {
...
static void construct_frame_header(char* buffer, int buffer_len, int payload_len)
{
	...
	*/
	buffer[buffer_len-1] = 0x70; // CRC byte 2
